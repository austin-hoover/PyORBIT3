...
from orbit.envelope_ode import KVEnvelope
from orbit.envelope_ode import KVEnvelopeMonitor
from orbit.envelope_ode import KVEnvelopeTracker
...
def main(args: argparse.Namespace) -> None:
    ...
    tracker = KVEnvelopeTracker(lattice, path_length_max=args.max_part_length)
    ...
    ax.plot(history["s"], history["rms_x"] * 1000.0, color="blue", alpha=1.0)
    ax.plot(history["s"], history["rms_y"] * 1000.0, color="red", alpha=1.0)
    ax.plot(history_unmatched["s"], history_unmatched["rms_x"] * 1000.0, color="blue", alpha=0.2)
    ax.plot(history_unmatched["s"], history_unmatched["rms_y"] * 1000.0, color="red", alpha=0.2)
